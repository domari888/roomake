...
RSpec.describe User, type: :model do
  describe 'バリデーション' do
    ...
    context 'avater の画像サイズが 5MB 以上のとき' do
      let(:user) { build(:user, avater: File.open("#{Rails.root}/public/images/fallback/rspec_size_test.jpg")) }
      it 'エラーが発生する' do
        expect(subject).to eq false
    ...
    context 'avater の拡張子が .jpeg .jpg .png 以外のとき' do
      let(:user) { build(:user, avater: File.open("#{Rails.root}/public/images/fallback/rspec_extension_test.tiff")) }
      it 'エラーが発生する' do
        expect(subject).to eq false
